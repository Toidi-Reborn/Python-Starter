...
def lookup(title = "", UPC = "", System = "", Value = "", Complete = "", NIB = ""):
	con = sqlite3.connect("gamesData.db")
	cur = con.cursor()
...
def search_command():
	...
	sNIBR1.delete(0,END)
	for rows in lookup(searchBox.get(),searchBox.get(),searchBox.get(),searchBox.get(),searchBox.get(),searchBox.get()):
...
scroll1.grid(row = 3, column = 3, rowspan = 10)

box1 = Listbox(window, bg = "silver", height = 23, width = 30)
...
breakLineEnd.grid(row=17, column=0)
